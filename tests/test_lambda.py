...
def test_health_check():
    ...
    res = myfristapplication.lambda_handler(event, None)
...
def test_get_task(table):
    created = json.loads(
        myfristapplication.lambda_handler(
            make_event("POST", "/tasks", body={"title": "T", "description": "D"}), None
        )["body"]
    )
    task_id = created["task_id"]

    res = myfristapplication.lambda_handler(make_event("GET", f"/tasks/{task_id}", task_id=task_id), None)
    assert res["statusCode"] == 200
    assert json.loads(res["body"])["task_id"] == task_id


def test_get_task_not_found(table):
    res = myfristapplication.lambda_handler(make_event("GET", "/tasks/ghost", task_id="ghost"), None)
...
def test_list_tasks(table):
    myfristapplication.lambda_handler(make_event("POST", "/tasks", body={"title": "T1", "description": "D1"}), None)
    myfristapplication.lambda_handler(make_event("POST", "/tasks", body={"title": "T2", "description": "D2"}), None)

    res = myfristapplication.lambda_handler(make_event("GET", "/tasks"), None)
    assert res["statusCode"] == 200
...
def test_update_task(table):
    ...
    res = myfristapplication.lambda_handler(
